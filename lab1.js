...
document.addEventListener("DOMContentLoaded", () => {
	...
	document.getElementById("run").addEventListener("click", () =>{
		...
		while(true)
		{
			...
			if (firstDer(f, sx) * firstDer(f, x1) < 0)
			{
				x0 = sx;
			}
			// else 
		...
		sx = sx < start || sx > end ? Math.abs(sx - end) < Math.abs(sx - start) ? end : start : sx
		...
		document.getElementById("answer").innerHTML += `На промежутке от ${start} до ${end} функция принимает экстремальное значение в точке: \nx* = ${sx} \nf(x*) = ${f(sx)}`;
